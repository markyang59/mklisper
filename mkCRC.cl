;-- CRC32 for DTV TS Packet
;-- CRC32 generator for TS Section
;-- Peter Yang
...
(defparameter *crc-table*
    (make-array
     ...
     :initial-contents
     '(#x00000000 #x04c11db7 #x09823b6e #x0d4326d9 #x130476dc #x17c56b6b
...
(defun crc (data)
  (let ((tcrc #xffffffff))
    (dotimes (i (length data))
      (setf tcrc (logxor
                  (ldb (byte 32 0) (ash tcrc 8))
                  (aref *crc-table* (logand (logxor (ash tcrc -24) (aref data i)) #xff))
                  )))
    tcrc
    ))
...
#|
;-- valid result is #x334ff8a0
(defun test-crc ()
  (let ((a (make-array 8 :element-type '(unsigned-byte 8)
                       :initial-contents '(#x00 #xB0 #x09 #x00 #x00 #xC1 #x00 #x00))))
    (crc a)))
|#